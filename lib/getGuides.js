 ...
 * Returns guides for the same destination (max 5)
 */
export function getRelatedGuides(currentSlug) {
  const allGuides = getAllGuides()
  const current = allGuides.find(g => g.slug === currentSlug)
  
  if (!current) return []
  
  // Find guides for the same destination, excluding current
  const related = allGuides
    .filter(g => 
      g.destination === current.destination && 
      g.slug !== currentSlug
    )
    .slice(0, 5)
